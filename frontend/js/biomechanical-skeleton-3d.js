...
class BiomechanicalSkeleton3D {
    constructor(containerId = 'biomechanics3DCanvasContainer', canvasId = 'biomechanics3DCanvas') {
        ...
        // Smooth position tracking for 17 keypoints in 3D (Vector3 + conf + valid)
        this.smoothedKeypoints = Array.from({ length: 17 }, () => ({
            pos: new THREE ? new THREE.Vector3() : { x: 0, y: 0, z: 0 },
            target: new THREE ? new THREE.Vector3() : { x: 0, y: 0, z: 0 },
            conf: 0,
            valid: false
        }));

        this.smoothingAlpha = 0.35; // EMA smoothing factor (0.0 to 1.0)
        this.lastPoseTime = 0;
        ...
        // Temporary Vector pool to eliminate per-frame allocations
        this.tempVecA = null;
        this.tempVecB = null;
        this.tempDir = null;
        this.tempAxis = null;
        this.midShoulder = null;
        this.midHip = null;
        ...
        // Colors
        this.colors = {
            jointNormal: 0x00F5FF,    // Glowing Cyan
            jointActive: 0xF5FF00,    // Neon Yellow
            jointWarning: 0xFF3366,   // Neon Pink/Red
            boneNormal: 0x7EDB7F,     // Light Neon Green
            boneWarning: 0xFF4D4D,    // Warning Red
            torsoSpine: 0x00E5FF,     // Bright Cyan
            grid: 0x222233
        };
    ...
    init() {
        ...
        if (typeof THREE === 'undefined') {
            ...
            this.showFallbackMessage('Three.js loading or unavailable...');
            return;
        }
        ...
        try {
            ...
            for (let i = 0; i < 17; i++) {
                this.smoothedKeypoints[i].pos = new THREE.Vector3(0, 1, 0);
                this.smoothedKeypoints[i].target = new THREE.Vector3(0, 1, 0);
            }
            ...
            this.materials.jointWarning = new THREE.MeshStandardMaterial({
                color: this.colors.jointWarning,
                roughness: 0.2,
                metalness: 0.5,
                emissive: 0x660011,
            ...
            // Create Bone Cylinder Meshes for defined connections
            ...
            // Create Spine Bone (Mid-Shoulders to Mid-Hips)
            ...
            // Create Neck Bone (Mid-Shoulders to Nose)
            ...
            // Set Standby Anatomical Stance until live keypoint stream starts
            this.setStandbyPose();
        } catch (err) {
    ...
    setStandbyPose() {
        // Anatomically proportional default stance
        const standbyKP = [
            { x: 0.50, y: 0.15, conf: 0.9 }, // 0: Nose
            { x: 0.48, y: 0.13, conf: 0.9 }, // 1: L Eye
            { x: 0.52, y: 0.13, conf: 0.9 }, // 2: R Eye
            { x: 0.45, y: 0.14, conf: 0.9 }, // 3: L Ear
            { x: 0.55, y: 0.14, conf: 0.9 }, // 4: R Ear
            { x: 0.40, y: 0.28, conf: 0.9 }, // 5: L Shoulder
            { x: 0.60, y: 0.28, conf: 0.9 }, // 6: R Shoulder
            { x: 0.35, y: 0.44, conf: 0.9 }, // 7: L Elbow
            { x: 0.65, y: 0.44, conf: 0.9 }, // 8: R Elbow
            { x: 0.32, y: 0.58, conf: 0.9 }, // 9: L Wrist
            { x: 0.68, y: 0.58, conf: 0.9 }, // 10: R Wrist
            { x: 0.43, y: 0.54, conf: 0.9 }, // 11: L Hip
            { x: 0.57, y: 0.54, conf: 0.9 }, // 12: R Hip
            { x: 0.43, y: 0.74, conf: 0.9 }, // 13: L Knee
            { x: 0.57, y: 0.74, conf: 0.9 }, // 14: R Knee
            { x: 0.43, y: 0.92, conf: 0.9 }, // 15: L Ankle
            { x: 0.57, y: 0.92, conf: 0.9 }  // 16: R Ankle
        ];
    ...
    /**
     * Map 2D normalized YOLO keypoints (0.0 to 1.0) into stable 3D coordinates.
     * Includes depth (Z) assumptions based on limb foreshortening & body tilt.
     */
    map2DTo3D(keypoints, exercise = 'squat') {
        ...
        // Extract raw 3D target coordinates for each keypoint
        for (let i = 0; i < 17; i++) {
            const kp = keypoints[i];
            const sk = this.smoothedKeypoints[i];

            if (kp && kp.conf >= 0.2) {
                // Mirror horizontally (1.0 - x) to match webcam preview orientation
                const tx = (0.5 - kp.x) * scaleX;
                const ty = (0.95 - kp.y) * scaleY + offsetY;
                let tz = 0.0;

                // Base depth assumption on torso angle & limb foreshortening
                sk.target.set(tx, ty, tz);
                sk.conf = kp.conf;
                sk.valid = true;
            } else {
                sk.valid = false;
        ...
        // Apply depth Z estimations if core joints are valid
        ...
        // 1. Torso Tilt Depth: if user rotates body, shoulder X separation shrinks
        ...
        // 2. Limb Flexion Depth (Knees forward during squats/lunges, Elbows during curls/presses)
        ...
        const legDepthDir = isLegExercise ? 1 : 0.5; // Knees move forward (+Z)

        // Left Leg & Right Leg Depth
        ...
        // Apply EMA Temporal Smoothing to eliminate keypoint jitter
    ...
    /**
     * Primary entry point called continuously from YOLO pose pipeline via window.onPoseDetected
     */
    ...
    /**
     * Zero-allocation cylinder aligner to connect point A to point B
     */
    ...
    renderFrame(customViolationsText = null) {
        ...
        const activeJoints = this.activeJointsPerExercise ? (this.activeJointsPerExercise[ex] || [11, 12, 13, 14]) : [11, 12, 13, 14];
        ...
        // Update 17 Joint Sphere Positions & Materials
        for (let i = 0; i < 17; i++) {
            ...
            if (mesh && sk) {
                if (sk.valid && sk.conf >= 0.2) {
                    ...
                    // Form violation joint highlighting
                    ...
                    if (isViolationJoint) {
                        mesh.material = this.materials.jointWarning;
                    } else if (activeJoints.includes(i)) {
                        // Key active movement joints glow yellow
                        mesh.material = this.materials.jointActive;
                    } else {
                        mesh.material = this.materials.jointNormal;
                    }
                } else {
                    mesh.visible = false;
                }
        ...
        // Update Spine Cylinder (Midpoint of Shoulders to Midpoint of Hips)
        ...
        // Update Neck Cylinder (Midpoint of Shoulders to Nose)
...
// Instantiate global 3D Skeleton Engine instance when DOM is ready
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (collapseBtn && bodyEl && iconEl) {
        collapseBtn.addEventListener('click', () => {
            ...
            if (isHidden && window.biomechanicalSkeleton3D) {
                setTimeout(() => window.biomechanicalSkeleton3D.onResize(), 50);
            }
        });
    }
});
